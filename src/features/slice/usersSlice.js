...
export const updateUser = createAsyncThunk(
	...
	async (userData, { rejectWithValue }) => {
		try {
			...
			console.log("Updating user with data:", values);
			const response = await axios.put(
				`${url}/users/update/${values.id}`,
				values,
				setHeaders()
			);
			return response.data;
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(fetchUserCreatedToday.fulfilled, (state, action) => {
				// Handle successful fetch of user created today
				state.userCreatedToday = action.payload;
				state.status = "success";
			})
			.addCase(fetchUserCreatedToday.rejected, (state) => {
				// Handle rejected fetch of user created today
				state.status = "rejected";
			});
	},
});
